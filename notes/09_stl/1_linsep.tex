...
\begin{frame}
		\underline{Solution:} 
			\begin{eqnarray}
				...
				&=& 2\sum_{k=0}^{N} \left({p-1 \atop k}\right)
				+ 2\sum_{k=0}^{N-1} \left({p-1 \atop k}\right) \\
				&=& 2 \underbrace{\left({p-1 \atop 0}\right)
						}_{1 = ({p \atop 0})}
					+ 2\sum_{k=1}^{N} \bigg[ 
						\underbrace{\left({p-1 \atop k}\right)
						+ \left({p-1 \atop k-1}\right)
							}_{\left({p \atop k}\right)}
					\bigg] 
				\quad=\quad \tilde C_{(p+1,N)} \,.
			\end{eqnarray}
\end{frame}
