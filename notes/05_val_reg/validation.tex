...
\underline{Recall}:
    ...
    \end{equation}
    \mode<presentation>{
    or mini-batch and stochastic online apprximations of $E^T_{[\vec w]}$ 
    }
    \mode<article>{
    ...
    We have also seen that $E^T$ can be furhter approximated by changing its ``scope'' to a set of smaples in a mini-batch or a single randomly selected sample (i.e. stochastic online learning)
...
We use the new hold-out set to \emph{validate} ($\{\vec x_{\text{val.}}^{(\beta)}, {y_T}_{\text{val.}}^{(\beta)}\}_{\beta=1}^{q}$) to esimtate the generalization error for this particular solution (i.e. $\widehat E^G_{[\vec w^*]})$
}
\slidesonly{
\begin{itemize}
...
\mode<article>{
Let us look at the following scenerio where we've trained 
}
...
\begin{frame}{\subsubsecname}
\slidesonly{\vspace{-2mm}}
\notesonly{Assuming has performed diagnostics of comapring $E^T$ and $\widehat E^G$ to identify if a model is overfitting or underfitting. How would one mitigate one or the other?\\
...
Although we've just talked about how increasing the size of the training set can help mitigate overfitting. Observing how $E^T$ and $\widehat E^G$ behave as a function of the training set size $p$ is a way of determining if an overfitting problem even exists. Keep in mind that $\widehat E^G$ would not directly depend on the size of training set as it is only evaluated on the hold-out set, which we won't change in size. Nevertheless, eveytime with train our nodel with a larger training set, we evaluate both $E^T$ and $\widehat E^G$ for this trained model.
}
...
\begin{figure}[ht]
     \centering
	\includegraphics[width=0.4\textwidth]{img/bias_var_dog}
     \mode<article>{
...
\begin{figure}[ht]
     \centering
	\includegraphics[width=0.4\textwidth]{img/bias_var_lambda}
     \mode<article>{
...
\begin{figure}[ht]
     \centering
	\includegraphics[width=0.4\textwidth]{img/bias_var_p_annot}
     \mode<article>{
...
- Other than disentangling high bias from high variance, we also get an assessment of how much 
 we would would gain in generalization performance if we paid to increase the training set by x\% (i.e. cost-benefit analysis).
}
...
\mode<article>{
Every cross-validation (inner loop) above yields a model with some generlization performance.
The parameters (incl. hyperparameter selection) of each cross validation can be different from the next.
The purpose of the outer loop is to estimate generlization performance that is less sensitive to the selection of the validation set, hyperparamerer selection and model selection procedure.
}
