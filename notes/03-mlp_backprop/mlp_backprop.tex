...
\begin{frame}\frametitle{\subsubsecname}
    ...
    \only<3,4>{
        ...
        \only<4>{
             ...
             {\color{red} \vec \delta^{2}} &=\; [\vec f^{2}]'({\color{darkgreen}\vec h^{2}}) \odot ({\color{red}\delta_1^{L}} \cdot {\widetilde{\vec w}_{1}^{L2}})
        ...
        \notesonly{
            \footnotetext{
            $\widetilde{\vec {w}}_{1}^{L2} \; \corresponds \; {\vec {w}}_{1}^{L2}$ without the bias node (k=0).\\
    ...
    \only<5,6>{
        ...
        %\only<6>{
            {\color{red} \vec \delta^{1}}
             &=
                ...
                ({\color{red} {\vec \delta^{2}}^{\top}}
                \kern-.5ex\cdot
                \widetilde{\vec {W}}^{21})^{\top} \notesonly{ \footnotemark }
                &(\text{\footnotesize \notesonly{bias} j=0  incl./k=0  excl.})\\
             &= 
                [\vec f^{1}]'({\color{darkgreen}h^{1}})
                \odot
        ...
        \notesonly{
            \footnotetext{
            $\widetilde{\vec {W}}^{21} \; \corresponds \; {\vec {W}}^{21}$ without the bias node (k=0) in each column ${\vec {w}}_{j}^{21}$.}
        }
    }
    \only<7,8>{
        \item $v=0$ (input layer):\\ \slidesonly{\vspace{-3mm}}
...
\begin{frame}\frametitle{Dimensions}

\begin{align}
0 &   & & \vec h^{0} \in \R^{N+1}\\ 
1 &   \vec w_{j}^{10}  \in \R^{N+1}    &  \vec {W}^{10} \in \R^{(N+1) \times N_{1}} &\vec h^{1} \in \R^{N_{1}+1}\\
2 &   \vec w_{i}^{21}  \in \R^{N_{1}+1}    &  \vec {W}^{21}  \in \R^{(N_{1}+1) \times N_{2}} &\vec h^{2} \in \R^{N_{2}+1}\\
L &\vec w_{1}^{L2}  \in \R^{N_{2}+1}    &  &\vec h^{L} \in \R\\  
\end{align}
