...
\begin{frame}\frametitle{The forward pass}
    \mode<presentation>{
    %\vspace{-3mm}
    %\begin{figure}[h]
    %\centering   
    %\includegraphics[height=4cm]{img/section1_fig14_fc}
    %\end{figure}
    %\vspace{-3mm}
    ...
    \only<1>{
        ...
        \begin{flalign}
            h^{0}_{j} &= x_{j}\;,\qquad j=1,\ldots,N&&\\
            h^{0}_{0} &= 1\qquad\quad(\text{bias node})&&\\
            \vec h^{0} &= \vec x\qquad\quad(\text{bias included})&&\\
            f^0_{j}(h) :\kern-0.5ex&= h \qquad\quad \text{ (identity function)} \;\Rightarrow\; \vec f^0(\vec h) = \vec x
        \end{flalign}
    }
    \only<2>{
        \item $v=1$ (1\textsuperscript{st} hidden layer):\\ \slidesonly{\vspace{-3mm}}
        \begin{flalign}
        h^{1}_{j} &= \kern-2ex\smallsum{(\mu,k) \in P(1,\,j)}{} \kern-2ex
	   			w_{jk}^{1\mu}\;  f_k^\mu\big( {h_k^\mu} \big)\;,\quad j=1,\ldots,N^{(1)}&&\\
                  &= \smallsum{k=0}{N} w_{jk}^{10}\;  f_k^0\big( {h_k^0} \big) = (\vec w_{j}^{10})^{\top} \vec h^0 \quad(\text{bias included})&&\\
        \vec h^{1} &= (\vec W^{10})^{\top} \vec h^0 \quad(\text{biases included})&&\\
        f^1_{j}(h) &= \text{e.g. }\tanh() \qquad j=1,\ldots,N^{(1)}&&
        \end{flalign}
    }
    \only<3,7,8>{
        \item $v=2$ (2\textsuperscript{nd} hidden layer):\\ \slidesonly{\vspace{-3mm}}
        \begin{flalign}
        h^{2}_{i} 
        \only<3>{&= \kern-2ex\smallsum{(\mu,j) \in P(2,\,i)}{} \kern-2ex
	   			w_{ij}^{2\mu}\;  f_j^\mu\big( {h_j^\mu} \big)\;,\quad i=1,\ldots,N^{(2)}&&\\
                }
                  &= \smallsum{j=0}{N^{(1)}} w_{ij}^{21}\;  f_j^1\big( {h_j^1} \big) = (\vec w_{i}^{21})^{\top} \vec f^{1}(\vec h^1) \quad(\text{bias included})&&\\
        \vec h^{2} &= (\vec W^{21})^{\top} \vec f^{1}(\vec h^1) \quad(\text{biases included})&&\\
        f^2_{i}(h) &= \text{e.g. }\tanh() \qquad i=1,\ldots,N^{(2)}&&
        \end{flalign}
    }
    \only<7,8>{
        ...
        \begin{flalign}
         ...
         &= 
            ...
            \kern-3ex\sum_{(v''\hspace{-1mm},\,k) \in C(2,\,i)}\kern-3ex
            {\color{red}\delta_k^{v''}} 
            {w}_{ki}^{2v'}
        \only<7>{&&}
        \only<8>{
         \;=\; 
            ...
            {\color{red}\delta_1^{L}} 
            {w}_{1i}^{L2}\\
         {\color{red} \vec \delta^{2}} &=\; (\vec w^{L2})^{\top}[\vec f^{2}]'(\vec h^{2})
            &&
        }
        \end{flalign}
    }
    \only<4,5,6>{
        ...
        \begin{flalign}
        h^{L}_{1} 
        \only<4>{&=\kern-2ex\smallsum{(\mu,i) \in P(L,\,1)}{} \kern-2ex
	   			w_{1i}^{L\mu}\;  f_i^\mu\big( {h_i^\mu} \big)\;&&\\
                }
                  &= \smallsum{i=0}{N^{(2)}} w_{1i}^{L2}\;  f_i^1\big( {h_i^1} \big) = (\vec w_{1}^{21})^{\top} \vec f^{2}(\vec h^2) \quad(\text{bias included})&&\\
    ...
    \only<5,6>{
        \slidesonly{\vspace{-8mm}}
        \begin{flalign}
         {\color{red}\delta^{v}_{i}}  &= \frac{\partial y(\vec{x}; \vec{w})} {{\color{blue}\partial h_1^{L}}}
					= [f_1^{L}]'({\color{blue} h_1^{L}})&&
        \end{flalign}
    }
    \end{itemize}
