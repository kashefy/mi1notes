...
\mode<article>{\eqref{eq:gradient_terms} shows us how the gradient breaks down into (1) an error term and (2) a term dependent on the type of model chosen:
}	
\begin{equation*}
...
The Backpropagation algorithm handles the computation of this term for neural networks such as the one depicted in \figref{fig:example_mlp}.
...
\begin{frame} \frametitle{Gradients in neural networks}
    ...
    \mode<article>{
    The Backpropagation algorithm is not tied to any specific architecture. We will therefore attempt to formulate how it works in the general sense.
