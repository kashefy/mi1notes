...
\begin{frame} \frametitle{Gradients in neural networks}
    ...
    \begin{enumerate}
     ...
     \item The contribution of $h^{v'}_{i}$ due to one of its inputs, namely neuron $(v,j)$ which is weighted by $w_{ij}^{v'v}$. We already have everythin to compute this term and that is by taking the definition of $h^{v'}_{i}$ in \eqref{eq:total_input_vi} and taking the derivative w.r.t. $w_{ij}^{v'v}$. 
...
\begin{frame}\frametitle{The Backpropagation algorithm}
    ...
    \begin{enumerate}
     ...
     \mode<article>{: Given some input $\vec x$ compute the activites of all neurons and the response of the network.}
     \item \textbf{backwards propagation}
