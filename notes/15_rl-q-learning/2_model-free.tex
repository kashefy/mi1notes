...
TD learning is the asynchronous on-line update of the value function one state at a time
	...
	\item \notesonly{TD learning} named after the difference $\Delta V_t$ in value function estimates (TD-error)
	...
	%\item TD learning performs value iteration \emph{on average}
		%\vspace{1mm}
		%\begin{itemize}
			%\item the expectation over all Markov chains 
			%that pass~$\vec x_i$~at~time~$t$~reads~$(\vec x^{(t)} = \vec x_i):$
			 %\vspace{2mm}
			%$$
		%\underbrace{\E\big\lbrack\tilde V^\pi_{t+1}(\vec x^{(t)})\big\rbrack
			%}_{\tilde v_i^{\pi(t+1)}}
		%\quad=\quad 
		%(1 - \lr)\underbrace{\E\big\lbrack \tilde V^\pi_t(\vec x^{(t)}) \big\rbrack
			%}_{\tilde v_i^{\pi(t)}}
		%\;+\; \lr \Big( \underbrace{\E\lbrack{\color{reward} r_t}\rbrack
		%+ \gamma \E\big\lbrack\tilde V^\pi_t({\color{trans}\vec x^{(t+1)}})\big\rbrack
...
%	\vspace{2mm}
%	\item {\em asynchronous online estimate} of 
...
\notesonly{To contract but to \underline{not} converge means that }\slidesonly{i.e.} for $t\rightarrow \infty$ TD learning will \underline{fluctuate} around the true value
...
Example with stochastic Markov chain with 10 states:
	\begin{itemize}
	...
	\begin{center}
		\includegraphics[width=0.999\textwidth]{img/rl_chain_valuecontraction}
		\mode<article>{
		\captionof{figure}{TD learning will continue fluctuating around the true value but not converge to it.}
		}
	\end{center}
...
Other model free approaches discussed in the lecture.%: Eligibility Traces \& TD($\lambda$), Batch Value Estimation, ...
