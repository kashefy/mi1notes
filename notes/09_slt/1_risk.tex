...
\begin{frame}\frametitle{The learning problem}
	...
	\begin{itemize}
        ...
        \notesonly{
        Therefore, $R_{[\vec w_0]}$ represents the risk value obtained from training the model using inifinite many points.
        }
...
\begin{frame}\frametitle{When does inductive learning through ERM work?}
	...
	\slidesonly{
		...
		\begin{figure}[h]
			...
			\mode<article>{
			\caption{
			Measuring the difference between empricial risk obtained from a finite site of $p$ points with risk obtained using inifitely many points.
			}
			\label{fig:randremp}
			} 
		\end{figure}
...
\begin{frame}\frametitle{\subsecname}\label{sec:convergence_erm}
			...
			Understanding \figref{fig:randremp} and what $P\bigg\{ 
					{
						\Big|R_{[\vec w_p]} - R_{[\vec w_0]}\Big| 
					}
			...
			\begin{itemize}
			\item We expect the model trained on $p$ points to have  some error $R_{\text{emp}}$ that is minimal for some $\vec w_p$ at which the emprical error is $R_{\text{emp}[\vec w_p]}$.
			...
			\item Now we repeat tranining these models but using a larger value for $p$.\\
