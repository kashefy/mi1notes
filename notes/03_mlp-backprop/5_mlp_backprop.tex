...
\begin{frame}\frametitle{\subsubsecname}
    ...
    \only<7,8>{
        ...
        \slidesonly{\only<7>{&&}}
        \only<8>{
            \smallsum{i=0}{N_{2}} w_{1i}^{L2}\;  f_i^{\textcolor{red}{2}}\big( {h_i^{\textcolor{red}{2}}} \big) = {\vec w_{1}^{\textcolor{red}{L2}}}^{\top} \vec f^{2}(\vec h^2) &(\text{bias included})&&\\
...
\begin{frame}\frametitle{\subsubsecname}
    \mode<presentation>{
    \only<-2>{
    \placeimage{7.4}{1}{img/section1_fig14_fc_nolayers_L}{width=6.7cm}
    }
    \only<3,4>{
    \placeimage{7.4}{1}{img/section1_fig14_fc_nolayers_v2}{width=6.7cm}
    }
    \only<5,6>{
    \placeimage{7.4}{1}{img/section1_fig14_fc_nolayers_v1}{width=6.7cm}
    }
    \only<7->{
    \placeimage{7.4}{1}{img/section1_fig14_fc_nolayers}{width=6.7cm}
    }
    ...
    \only<1,2>{
        ...
        \begin{flalign}
            ...
            {\color{red}\delta^{L}_{1}}  &= \frac{\partial y(\vec{x}; \vec{w})} {\partial{\color{darkgreen} h_1^{L}}} =
        \only<2>{
                [f_1^{L}]'({\color{darkgreen} h_1^{L}}) \cdot \textcolor{gray}{1}
                & (\textcolor{gray}{C(L,1) \text{ is an empty set}})
        }
        \end{flalign}
    }
    ...
    \only<3,4>{
        ...
        \begin{flalign}
            ...
            f^2_{\color{blue}0}(h) :\kern-0.5ex&= h &\text{(identity for \textcolor{blue}{bias})}\\
        ...
        \only<4>{
                ...
                &\\
             {\color{red} \vec \delta^{2}} &=\; [\vec f^{2}]'({\color{darkgreen}\widetilde{\vec h}^{2}})
             \odot
             (
             {\widetilde{\vec w}_{1}^{L2}}
             \cdot
             {\color{red}\delta_1^{L}}
             )
        \notesonly{ \footnotemark }\\
        %\notesonly{ \footnotemark }
             {\color{red}\delta^{2}_{\color{blue}0}} &=
                \; 1 \cdot \,
                {\color{red}\delta_1^{L}} \,
                {w}_{1{\color{blue}0}}^{L2} 
                & (\text{local error for \textcolor{blue}{bias}})\\
        }
        \end{flalign}
        \notesonly{
            \footnotetext{
    ...
    \only<5,6>{
        ...
        \begin{flalign}
            f^1_{j}(h) :\kern-0.5ex&= \text{e.g. }\tanh(h),  &j=1,\ldots,N_{1}\\ 
            f^1_{\color{magenta}0}(h) :\kern-0.5ex&= h &\text{(identity for \textcolor{magenta}{bias})}&&\\
        ...
        \only<6>{
                ...
                &(\text{\textcolor{blue}{bias} }\textcolor{blue}{k=0}\text{  excl.})\\%TODO: vector notation for this
        ...
        %\only<6>{
            {\color{red} \vec \delta^{1}}
             ...
             %&= 
                ...
                {\color{red}\delta^{1}_{\color{magenta}0}}
				&=
				\; 1 \, \cdot
                \kern-.5ex\sum_{{k=1}}^{N_{2}}\kern-.5ex
                {\color{red}\delta_k^{2}} \,
                {w}_{{k}{\color{magenta}0}}^{21}
                &(\text{local error for \textcolor{magenta}{bias}})\\
        }
        \end{flalign}
        \notesonly{
            \footnotetext{
    ...
    \only<7,8>{
        ...
        \begin{flalign}
                f^0_{j}(h) :\kern-0.5ex&= h & \text{ (identity function)}&&\\
                \vec f^0(\vec h) :\kern-0.5ex&= \vec x &\text{ (bias } x_0 \text{ included)}
                \\
...
\begin{center}
	\includegraphics[width=0.4\textwidth]{img/meme_learning_backprop}
\end{center}
	}
}
\end{frame}
