...
\begin{enumerate}
    \item<only@1-> perfect regression solution, i.e. zero error $\varepsilon$-sensitive cost,
...
\begin{enumerate}
    ...
    \item<only@2,3> constrained regression problem that allows for errors (with slack variables $\varphi_\alpha$)\\
...
\slidesonly{\vspace{-3mm}}
        ...
        \only<2>{
        \question{How do prevent solutions with arbitrarily large $\varphi_\alpha, \varphi_\alpha^*$ that prevent any violation of the constraints?}\\
