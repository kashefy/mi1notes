...
\StickyNote[1.cm]{
	...
	\begin{equation}
	\label{eq:ephi}
		e^{(\alpha)} = \frac{1}{2} \, \left( y(\vec x^{(\alpha)}; \vec w)- y^{(\alpha)}_{T}\right)^{2}
	\end{equation}
	\begin{equation}
	\label{eq:ephi}
		y = \vec w^{\top}\vec x
...
\frac{1}{p} \sum_{\alpha=1}^{p} \;
{\color{blue}
\frac{\partial e^{(\alpha)}}{\partial y(\vec x^{(\alpha)};\vec w)}
} 
\cdot
{\color{orange}
...
\frac{1}{p} \sum_{\alpha=1}^{p} \;
{\color{blue}
\big( y(\vec x^{(\alpha)}; \vec w) - y_{T}^{(\alpha)} \big)
}
\cdot
{\color{orange}
...
\visible<1->{
\begin{align}
\vec g^R :=  \visible<2>{\frac{\partial}{\partial \vec w} R_{[\vec w]}
