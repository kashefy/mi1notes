...
\mode<article>{$\phi(\vec x)$ can transform our $N$-dim input $\vec x$ into the feature space spanned by the $k$ basis functions postioned at $\vec t_1,\ldots,\vec t_k$.
}
...
\mode<presentation>{
\begin{equation}
\vec y_{\text{Parzen}}(\vec x) := \frac{1}{Z} \sum_{\alpha=1}^{p} \vec y_T^{(\alpha)} \cdot \kappa(\vec x, \vec x^{(\alpha)})
\qquad \text{(Parzen window classifier)}
\end{equation}
\begin{equation}
    \vec y_{\text{RBF-Net}}(\vec \phi; \vec w) = \sum_{i=0}^{k} w_{i} \phi_{i}(\vec x) = \vec w^{\top} \vec \phi \qquad \text{(RBF-Network)}
\end{equation}
}
...
- An RBF-Network can be constructed and parameterized to act as a Parzen window classifier by placing the a ``representative'/centroid over each point.
...
\begin{tabular}[c]{@{}l@{}}total input of a hidden neuron\\determined via inner product\\$\vec w_i^\top x$\end{tabular}                  & \begin{tabular}[c]{@{}l@{}}total input is based on a Euclidean dsitance\end{tabular} \\ 
\hline\pause
